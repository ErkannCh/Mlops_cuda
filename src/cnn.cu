...
void SimpleCNN::forward_device(const float* d_input_external, float* d_output_external) {
    ...
    gpu_conv2d_relu_naive(d_input_external, d_conv_w, d_conv_b, d_conv_out, N, C_in, H, W, C_out_conv, K, H_out_, W_out_);
    ...
    gpu_feedforward_layer(d_fc_in, d_fc_w, d_fc_b, d_output_external, N, fc_in_dim, cfg_.fc_out);
}

void SimpleCNN::forward(const float* input_host, float* output_host) {
    allocate_device_buffers();

    checkCuda(cudaMemcpy(d_input, input_host, sizeof(float) * cfg_.N * cfg_.C_in * cfg_.H * cfg_.W, cudaMemcpyHostToDevice), "memcpy input_host->d_input");

    forward_device(d_input, d_output);

    checkCuda(cudaMemcpy(output_host, d_output, sizeof(float) * cfg_.N * cfg_.fc_out, cudaMemcpyDeviceToHost), "memcpy d_output->output_host");
}
