...
void SimpleCNN::allocate_device_buffers() {
    if (!d_input) {
        int N = cfg_.N;
        int fc_in_dim = cfg_.C_out_conv * H_out_ * W_out_;
        checkCuda(cudaMalloc(&d_input,
                             sizeof(float) * N * cfg_.C_in * cfg_.H * cfg_.W),
                  "malloc d_input");
        checkCuda(cudaMalloc(&d_conv_out,
                             sizeof(float) * N * cfg_.C_out_conv * H_out_ * W_out_),
                  "malloc d_conv_out");
        checkCuda(cudaMalloc(&d_fc_in,
                             sizeof(float) * N * fc_in_dim),
                  "malloc d_fc_in");
        checkCuda(cudaMalloc(&d_output,
                             sizeof(float) * N * cfg_.fc_out),
                  "malloc d_output");
    }
}
...
void SimpleCNN::copy_weights_to_device() {
    ...
    checkCuda(cudaMemcpy(d_conv_w, h_conv_w.data(),
                         sizeof(float) * conv_w_size,
                         cudaMemcpyHostToDevice),
              "memcpy conv_w");
    checkCuda(cudaMemcpy(d_conv_b, h_conv_b.data(),
                         sizeof(float) * conv_b_size,
                         cudaMemcpyHostToDevice),
              "memcpy conv_b");
    checkCuda(cudaMemcpy(d_fc_w, h_fc_w.data(),
                         sizeof(float) * fc_w_size,
                         cudaMemcpyHostToDevice),
              "memcpy fc_w");
    checkCuda(cudaMemcpy(d_fc_b, h_fc_b.data(),
                         sizeof(float) * fc_b_size,
                         cudaMemcpyHostToDevice),
              "memcpy fc_b");
}
...
void SimpleCNN::load_weights_from_file(const std::string& path) {
    ...
    if (N != cfg_.N || C_in != cfg_.C_in || H != cfg_.H || W != cfg_.W ||
        C_out_conv != cfg_.C_out_conv || K != cfg_.K || fc_out != cfg_.fc_out) {
...
// Flatten: (N, C_out_conv, H_out, W_out) -> (N, C_out_conv*H_out*W_out)
static __global__ void flatten_nchw_kernel(const float* input,
                                           float* output,
...
void SimpleCNN::forward_device(const float* d_input_external, float* d_output_external) {
    ...
    flatten_nchw_kernel<<<gridSize, blockSize>>>(
        d_conv_out, d_fc_in, N, C_out_conv, H_out_, W_out_
    );
