...
void MLP::allocate_device_buffers(int batch_size) {
    ...
    checkCuda(cudaMalloc(&d_input, sizeof(float) * batch_size * config_.input_dim),
              "cudaMalloc d_input");
    checkCuda(cudaMalloc(&d_hidden, sizeof(float) * batch_size * config_.hidden_dim),
              "cudaMalloc d_hidden");
    checkCuda(cudaMalloc(&d_output, sizeof(float) * batch_size * config_.output_dim),
              "cudaMalloc d_output");
...
void MLP::copy_weights_to_device() {
    ...
    checkCuda(cudaMemcpy(d_W1, h_W1.data(),
                         sizeof(float) * h_W1.size(),
                         cudaMemcpyHostToDevice),
              "memcpy W1");
    checkCuda(cudaMemcpy(d_b1, h_b1.data(),
                         sizeof(float) * h_b1.size(),
                         cudaMemcpyHostToDevice),
              "memcpy b1");
    checkCuda(cudaMemcpy(d_W2, h_W2.data(),
                         sizeof(float) * h_W2.size(),
                         cudaMemcpyHostToDevice),
              "memcpy W2");
    checkCuda(cudaMemcpy(d_b2, h_b2.data(),
                         sizeof(float) * h_b2.size(),
                         cudaMemcpyHostToDevice),
              "memcpy b2");
}
...
void MLP::load_weights_from_file(const std::string& path) {
    ...
    if (in_dim != config_.input_dim ||
        hid_dim != config_.hidden_dim ||
        out_dim != config_.output_dim) {
...
void MLP::forward(const float* input_host, float* output_host, int batch_size) {
    ...
    // Couche 1: hidden = ReLU(input * W1^T + b1)
    ...
    // Couche 2: output = hidden * W2^T + b2 (pas de ReLU pour l'exemple)
    gpu_feedforward_layer(
        d_hidden, d_W2, d_b2,
        d_output,
        batch_size,
